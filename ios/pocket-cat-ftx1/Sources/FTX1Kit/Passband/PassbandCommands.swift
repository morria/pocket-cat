...
// yaesu-cat-ftx1.md "Passband commands" (researched from Hamlib,
...
extension TransceiverSession {
    // MARK: - Reply plumbing (mirrors FTX1Commands)
    ...
    public func readIFShift() async throws -> Int {
        ...
        guard body.count == 6, let hz = Int(body.dropFirst()) else {
            throw FTX1Error.malformedReply(command: "IS0;",
                                            reply: String(body))
        }
        return hz
    }
    ...
    public func readWidthIndex() async throws -> Int {
        // FTX-1 replies with the extra "on" digit: SH0 <on:1> <nn> ;
        ...
        guard body.count == 2 || body.count == 3,
            ...
            throw FTX1Error.malformedReply(command: "SH0;",
                                            reply: String(body))
        }
        return index
    }
    ...
    public func readNarrow() async throws -> Bool {
        ...
        guard body.count == 1, let flag = body.first else {
            throw FTX1Error.malformedReply(command: "NA0;",
                                            reply: String(body))
        }
        return flag != "0"
    }
    ...
    public func readNotchEnabled() async throws -> Bool {
        ...
        guard body.count == 3, let value = Int(body) else {
            throw FTX1Error.malformedReply(command: "BP00;",
                                            reply: String(body))
        }
        return value != 0
    }
    ...
    public func readNotchHz() async throws -> Int {
        ...
        guard body.count == 3, let tens = Int(body) else {
            throw FTX1Error.malformedReply(command: "BP01;",
                                            reply: String(body))
        }
        return tens * 10
    }
    ...
    public func readContourEnabled() async throws -> Bool {
        ...
        guard body.count == 4, let value = Int(body) else {
            throw FTX1Error.malformedReply(command: "CO00;",
                                            reply: String(body))
        }
        return value != 0
    }
    ...
    public func readContourHz() async throws -> Int {
        ...
        guard body.count == 4, let hz = Int(body) else {
            throw FTX1Error.malformedReply(command: "CO01;",
                                            reply: String(body))
        }
        return hz
    }
    ...
    public func readAutoNotch() async throws -> Bool {
        ...
        guard body.count == 1, let flag = body.first else {
            throw FTX1Error.malformedReply(command: "BC0;",
                                            reply: String(body))
        }
        return flag != "0"
    }
