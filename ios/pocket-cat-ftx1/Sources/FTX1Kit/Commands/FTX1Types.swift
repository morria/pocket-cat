// FTX-1-specific value types for the command surface that CATBridgeKit
// leaves untyped. Wire formats per docs/ftx1-cat-commands.md.
...
/// `RM` meter selects.
public enum FTX1Meter: Character, Sendable, CaseIterable {
...
public enum FTX1Band: String, Sendable, CaseIterable, Identifiable {
    case m160 = "00"
    case m80 = "01"
    case m40 = "03"
    case m30 = "04"
...
/// Operating modes the FTX-1 actually supports (subset of CATBridgeKit's
/// `OperatingMode` — the radio has no C4FM/DATA-FM and rejects them).
public enum FTX1Mode {
    /// The FTX-1 is a full-mode SDR, so the newcat codes the FT-891 leaves
    /// unused — `A` DATA-FM, `E` C4FM, `F` DATA-FM-N — are live here.
    public static let all: [OperatingMode] = [
        .lsb, .usb, .cw, .cwReverse, .am, .amNarrow, .fm, .fmNarrow,
        .dataLSB, .dataUSB, .dataFM, .dataFMNarrow, .rtty, .rttyReverse,
        .c4fm,
    ]

    /// Yaesu MD wire codes for the modes the FTX-1 supports
    /// (docs/ftx1-cat-commands.md §3.2; code "A"/"E"/"F" unused here).
    public static let codeForMode: [OperatingMode: Character] = [
        ...
        .dataFM: "A", .fmNarrow: "B", .dataUSB: "C", .amNarrow: "D",
        .c4fm: "E", .dataFMNarrow: "F",
    ]
...
public enum FTX1Error: Error, Equatable {
