...
// FTX-1 state (tuner, split, clarifier, TX meters, menu cache) on top of
...
public final class RigController {
    ...
    private static let lastBridgeKey = "ftx1.lastBridgeID"
    ...
    public func selectBand(_ band: FTX1Band) async {
        guard let session else { return }
        do {
    ...
    public func readMenuDigits(_ exNumber: String) async throws -> String {
        ...
        return try await session.readMenuDigits(exNumber)
    }

    public func writeMenuDigits(_ exNumber: String,
                                digits: String) async throws {
        guard let session else { throw CATBridgeError.notReady }
        try await session.writeMenuDigits(exNumber, digits: digits)
    }
    ...
    public func refreshKeyerSpeed() async {
        guard let session else { return }
    ...
    /// Hands text to the radio's memory keyer (`KY`). Buffered, not
    /// real-time keying, so link latency doesn't affect what goes out.
