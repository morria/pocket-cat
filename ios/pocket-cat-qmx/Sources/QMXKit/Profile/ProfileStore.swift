...
public actor ProfileStore {
    ...
    public func list() throws -> [StoredProfile] {
        ...
        return urls
            .filter { $0.pathExtension == QMXProfile.fileExtension }
            .compactMap { url in
                guard let data = coordinatedRead(url),
                      let profile = try? QMXProfile.decode(data) else {
                    return nil
                }
    ...
    @discardableResult
    public func save(_ profile: QMXProfile) throws -> StoredProfile {
    ...
    private func uniqueURL(stem: String) -> URL {
        ...
        var candidate = directory.appendingPathComponent(stem)
            .appendingPathExtension(QMXProfile.fileExtension)
        var counter = 2
