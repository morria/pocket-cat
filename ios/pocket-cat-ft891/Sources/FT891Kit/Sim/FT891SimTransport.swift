// A BridgeTransport that impersonates the Pocket Cat bridge with an FT-891
...
// FT-891 personality, extended with the full EX menu table.

import CATBridgeCore
import Foundation

public actor FT891SimTransport: BridgeTransport {
    ...
    public var rigState: FT891SimRig { rig }

    public func setRig(_ transform: @Sendable (inout FT891SimRig) -> Void) {
        transform(&rig)
    }
...
public struct FT891SimRig: Sendable {
    // Operating state
    ...
    /// EX menu store: exNumber → raw digit string, seeded from catalog
    /// defaults.
    public var menu: [String: String] = {
        var store: [String: String] = [:]
        for item in MenuCatalog.items {
            store[item.exNumber] = (try? item.encode(item.defaultValue)) ?? "0"
        }
        // Consistent with `catRate = 38400` below (the recommended setup,
        // not the 4800 factory default the catalog records).
        store["0506"] = "3"
        return store
    }()
    ...
    private mutating func respondEX(body: String) -> String? {
        ...
        guard let stored = menu[number] else { return "?;" }
        let value = String(rest.dropFirst(4))
        if value.isEmpty { return "EX\(number)\(stored);" }
        guard let item = MenuCatalog.byEXNumber[number],
              item.isWritable, (try? item.decode(value)) != nil else {
            return "?;"
        }
        menu[number] = value
        return nil
